...
const selectOrder = (id_order) => {
  return Pool.query(`SELECT * FROM order_list WHERE id_order = ${id_order}`);
};

const insertOrder = (data) => {
  const { id_order, id_product, quantity_order, date_order, } = data;
  return Pool.query(`INSERT INTO order_list(id_order, id_product, quantity_order, date_order) VALUES(${id_order},${id_product},${quantity_order}, '${date_order}')`);
};
